...
from mmdp.models.architectures.generator import UnetGenerator
from mmdp.models.losses import L1Loss, MSELoss
...
class TestPixelwiseLosses:
    ...
    def test_pixelwise_losses(self):
        ...
        # test l1 loss
        l1_loss = L1Loss(loss_weight=1.0, reduction="mean", data_info=self.data_info)
...
class TestGaussianKLDLoss:
    ...
    def test_gaussian_kld_loss(self):
        ...
        # test reduction --> mean
        gaussian_kld_loss = GaussianKLDLoss(data_info=self.data_info, reduction="mean")
        ...
        loss = gaussian_kld_loss(self.output_dict)
        num_elements = self.tar_shape[1] * self.tar_shape[2] * self.tar_shape[3]
        ...
        weight = torch.randn(*self.tar_shape)
        gaussian_kld_loss = GaussianKLDLoss(
            loss_weight=weight, data_info=self.data_info, reduction="batchmean"
        )
        loss = gaussian_kld_loss(self.output_dict)
...
def test_approx_gaussian_cdf():
    ...
    assert torch.allclose(approx_gaussian_cdf(pos), gaussian_dist.cdf(pos), atol=1e-3)


class TestDistLoss:
    @classmethod
    def setup_class(cls):
        cls.data_info = dict(mean="mean_pred", logvar="logvar_pred", x="real_imgs")
        cls.tar_shape = [2, 2, 4, 4]
