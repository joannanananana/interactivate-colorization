...
from mmdp.models.builder import build_module
from mmdp.models.losses.pixelwise_loss import (
...
class TestDDPMVLBLoss:
    ...
    def setup_class(cls):
        ...
        cls.config = dict(
            ...
            log_cfgs=[
                dict(type="quartile", prefix_name="loss_vlb", total_timesteps=4),
                dict(type="name"),
            ],
        )
        ...
        cls.loss_gaussian_kld = GaussianKLDLoss(
            data_info=cls.gaussian_kld_data_info, reduction="flatmean", base="2"
        )(cls.output_dict)
        cls.loss_disc_likelihood = DiscretizedGaussianLogLikelihoodLoss(
            data_info=cls.disc_log_likelihood_data_info, reduction="flatmean", base="2"
        )(cls.output_dict)

        cls.loss_manually = (
            -cls.loss_disc_likelihood[0] + cls.loss_gaussian_kld[1:].sum()
        ) / 4

        # TODO: unit test for sampler would be add later
        cls.weight = torch.rand(
            4,
        )
    ...
    def test_vlb_loss(self):
        ...
        loss_weighted_manually = (
            ...
            + (self.loss_gaussian_kld * weight)[1:].sum()
        ) / 4
...
class TestDDPMMSELoss:
    ...
    def setup_class(cls):
        ...
        cls.config = dict(
            ...
            log_cfgs=dict(type="quartile", prefix_name="loss_mse", total_timesteps=4),
        )
        ...
        cls.output_dict = dict(
            eps_t_pred=cls.eps_t_pred, noise=cls.noise, meta_info=None, timesteps=cls.t
        )
        ...
        for idx in range(cls.tar_shape[0]):
            t = cls.t[idx]
            weight = cls.weight[t]
            output_dict_ = dict(eps_t_pred=cls.eps_t_pred[t], noise=cls.noise[t])
            cls.loss_manually += MSELoss(data_info=cls.data_info)(output_dict_) * weight
        cls.loss_manually /= 4
