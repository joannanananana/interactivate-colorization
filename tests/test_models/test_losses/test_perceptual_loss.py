...
from mmdp.models import PerceptualLoss, PerceptualVGG, TransferalPerceptualLoss
...
def test_perceptual_loss(init_weights):
    if torch.cuda.is_available():
        ...
        loss_percep = PerceptualLoss(
            layer_weights={"0": 1.0}, perceptual_weight=0.0
        ).cuda()
        x = torch.randn(1, 3, 16, 16).cuda()
        gt = torch.randn(1, 3, 16, 16).cuda()
        percep, style = loss_percep(x, gt)
        ...
        loss_percep = PerceptualLoss(
            layer_weights={"0": 1.0}, style_weight=0.0, criterion="mse"
        ).cuda()
        x = torch.randn(1, 3, 16, 16).cuda()
        gt = torch.randn(1, 3, 16, 16).cuda()
        percep, style = loss_percep(x, gt)
        ...
        loss_percep = PerceptualLoss(
            layer_weights={"0": 1.0}, layer_weights_style={"1": 1.0}
        ).cuda()
        x = torch.randn(1, 3, 16, 16).cuda()
        gt = torch.randn(1, 3, 16, 16).cuda()
        percep, style = loss_percep(x, gt)
