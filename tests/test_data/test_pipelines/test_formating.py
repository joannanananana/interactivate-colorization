...
from mmdp.datasets.pipelines import Collect, ImageToTensor, ToTensor
...
def test_image_to_tensor():
    ...
    assert repr(image_to_tensor) == (
        image_to_tensor.__class__.__name__ + f"(keys={keys}, to_float32={to_float32})"
    )
...
def test_collect():
    ...
    assert repr(collect) == (
        collect.__class__.__name__ + f"(keys={keys}, meta_keys={collect.meta_keys})"
    )
