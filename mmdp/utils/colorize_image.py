...
def lab2rgb_transpose(img_l, img_ab):
    """INPUTS img_l     1xXxX     [0,100] img_ab     2xXxX     [-100,100]
    OUTPUTS returned value is XxXx3."""
...
def rgb2lab_transpose(img_rgb):
    """INPUTS img_rgb XxXx3 OUTPUTS returned value is 3xXxX."""
...
class ColorizeImageTorch(ColorizeImageBase):
    ...
    def prep_net(self, gpu_id=None, path="", dist=False):
        ...
        self.net = SIGGRAPHGenerator(input_nc=4, output_nc=2)
        ...
        for key in list(state_dict.keys()):
            ...
            y = state_dict[x]
            try:
                x = x.split("generator.")[1]
                state_dict_new[x] = y
            except ValueError:
                pass

        self.net.load_state_dict(state_dict_new, strict=True)
    ...
    # ***** Call forward *****
    def net_forward(self, input_ab, input_mask, color_mode=True):
        # INPUTS
        #     ab         2xXxX     input color patches (non-normalized)
        #     mask     1xXxX    input mask, indicating which points have been provided
        # assumes self.img_l_mc has been set
...
class ColorizeImageTorchDist(ColorizeImageTorch):
    ...
    def get_ab_reccs(self, h, w, K=5, N=25000, return_conf=False):
        """Recommended colors at point (h,w) Call this after calling
        net_forward."""
        ...
        cluster_per = (
            1.0 * k_label_cnt[k_inds] / N
        )  # percentage of points within cluster
