...
from mmdp.core.ddp_wrapper import DistributedDataParallelWrapper
from mmdp.core.optimizer import build_optimizers
from mmdp.core.runners.apex_amp_utils import apex_amp_initialize
...
def train_model(
    ...
    if cfg.get("apex_amp", None):
        assert distributed, "Currently, apex.amp is only supported with DDP training."
        model = model.cuda()
    ...
    if distributed:
        ...
        if use_ddp_wrapper:
            mmcv.print_log("Use DDP Wrapper.", "mmdp")
