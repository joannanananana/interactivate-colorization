...
# from mmdp.models import BaseTranslationModel, build_model
...
@torch.no_grad()
def sample_uncoditional_model(
    model, num_samples=16, num_batches=4, sample_model="ema", **kwargs
):
...
@torch.no_grad()
def sample_conditional_model(
    model, num_samples=16, num_batches=4, sample_model="ema", label=None, **kwargs
):
    ...
    for batches, labels in zip(batches_list, label_list):
        res = model.sample_from_noise(
            None, num_batches=batches, label=labels, sample_model=sample_model, **kwargs
        )
        res_list.append(res.cpu())
...
def sample_img2img_model(model, image_path, target_domain=None, image=None, **kwargs):
    ...
    with torch.no_grad():
        results = model(
            source_image, test_mode=True, target_domain=target_domain, **kwargs
        )
