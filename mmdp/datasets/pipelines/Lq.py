...
import mmdp.utils.utils_image as util
...
def add_JPEG_noise(img):
    quality_factor = random.randint(5, 20)
...
def degradation_bsrgan(img, sf=4, lq_patchsize=32, isp_model=None, use_crop=True):
    """
    This is the degradation model of BSRGAN from the paper
    "Designing a Practical Degradation Model for Deep Blind Image Super-Resolution"
    ----------
    img: H * W * C, [0, 1], its size should be large than (lq_patchsizexsf)x(lq_patchsizexsf)
    sf: scale factor
...
class Lq_util:
    def __init__(self, key, sf=4, **kwargs):
        self.key = key
        self.sf = sf
    ...
    def Bicubic_degradation(self, x, sf=4):
        """
        Args:
            x: HxWxC image, [0, 1]
    ...
    def __call__(self, results):
        ...
        img = self.Add_blur(results[self.key])
        img = self.Bicubic_degradation(img, self.sf)
        results[self.key] = self.Add_blur(img)
...
class Lq_downsample:
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        results[self.key] = self.Bicubic_degradation(results[self.key], self.sf)
...
class Lq_degradation_bsrgan:
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        img = util.uint2single(results[self.key])
        img, _ = degradation_bsrgan(img, self.sf, use_crop=False)
        results[self.key] = cv2.resize(
            util.single2uint(img),
...
@PIPELINES.register_module()
class Lq_degradation_bsrgan_plus:
    def __init__(self, key, sf=4, **kwargs):
        self.key = key
        self.sf = sf
    ...
    def __call__(self, results):
        ...
        """
        img = util.uint2single(results[self.key])
        img, _ = degradation_bsrgan_plus(img, self.sf, crop=False)
        results[self.key] = cv2.resize(
            util.single2uint(img),
            (int(img.shape[1]), int(img.shape[0])),
            interpolation=0,
