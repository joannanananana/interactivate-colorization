        ...
        for item in data_list:
            ...
            if self.is_pad_zeros:
                ...
                if crop_y_offset > 0 or crop_x_offset > 0:
                    pad_width = [
                        (2 * crop_y_offset, 2 * crop_y_offset),
...
class PairedRandomCrop:
    ...
    def __call__(self, results):
        ...
        results["gt"] = [
            v[
                top_gt : top_gt + self.gt_patch_size,
...
class RandomResizedCrop(object):
    ...
    def __init__(
        ...
        if not mmcv.is_tuple_of(scale, float):
            raise TypeError(
                '"scale" must be a tuple of float, ' f"but got {type(scale)}"
            )
        if not mmcv.is_tuple_of(ratio, float):
            raise TypeError(
                '"ratio" must be a tuple of float, ' f"but got {type(ratio)}"
            )
