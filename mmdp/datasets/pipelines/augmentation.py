...
class Flip:
    ...
    def __repr__(self):
        ...
        repr_str += (
            f"(keys={self.keys}, flip_ratio={self.flip_ratio}, "
...
class Resize:
    ...
    def __init__(
        ...
        if size_factor:
            ...
            assert keep_ratio is False, (
                "When size_factor is used, " "keep_ratio should be False."
            )
        if max_size:
        ...
        if isinstance(scale, float):
            ...
            raise TypeError(
                f"Scale must be None, float or tuple of int, but got " f"{type(scale)}."
            )
        self.keys = keys
...
class NumpyPad:
    ...
    def __repr__(self) -> str:
        repr_str = self.__class__.__name__
        repr_str += f"(keys={self.keys}, padding={self.padding}, kwargs={self.kwargs})"
        return repr_str
...
class RandomImgNoise:
    ...
    def __init__(
        self, keys, lower_bound=0, upper_bound=1 / 128.0, distribution="uniform"
    ):
        assert keys, "Keys should not be empty."
        ...
        if distribution not in ["uniform", "normal"]:
            raise KeyError(
                "Only support 'uniform' distribution and "
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += (
            f"(keys={self.keys}, lower_bound={self.lower_bound}, "
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            ...
            y1 = (
                ...
                else np.random.randint(0, img_height - crop_size)
            )
            x1 = (
                0
                if img_width == crop_size
                else np.random.randint(0, img_width - crop_size)
            )
...
class CenterCropLongEdge:
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            ...
            crop_size = min(img_height, img_width)
            y1 = (
                0 if img_height == crop_size else int(round(img_height - crop_size) / 2)
            )
            x1 = 0 if img_width == crop_size else int(round(img_width - crop_size) / 2)
            y2 = y1 + crop_size - 1
            x2 = x1 + crop_size - 1
