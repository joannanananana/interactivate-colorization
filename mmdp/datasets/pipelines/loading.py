    ...
    def __call__(self, results):
        ...
        if self.convert_to is not None:
            if self.channel_order == "bgr" and self.convert_to.lower() == "y":
                ...
                raise ValueError(
                    'Currently support only "bgr2ycbcr" or ' '"bgr2ycbcr".'
                )
            if img.ndim == 2:
...
class LoadPairedImageFromFile(LoadImageFromFile):
    ...
    def __call__(self, results):
        ...
        if self.paired:
            ...
            img_b = img
            img_a = mmcv.image.bgr2gray(img, keepdim=True)
            img_a = mmcv.image.gray2bgr(img_a)
...
@PIPELINES.register_module()
class LoadLabImageFromFile(LoadImageFromFile):
    """Load a images from file and change it into gray and Lab pair.
    ...
    def __call__(self, results):
        ...
        # 使用 pair 数据集的时候只使用有色彩的部分
        # w = img.shape[1]
        # new_w = w // 2
        # img = img[:, new_w:, :]

        img_Lab = cv2.cvtColor(img, cv2.COLOR_BGR2Lab)
        img_a = img_Lab[:, :, 0][:, :, np.newaxis].repeat(3, axis=2)
        img_b = img_Lab[:, :, 1:]
