...
def create_real_pyramid(real, min_size, max_size, scale_factor_init):
    ...
    scale_factor = np.power(
        min_size / (min(real_max.shape[0], real_max.shape[1])), 1 / (stop_scale)
    )
...
class SinGANDataset(Dataset):
    ...
    def __init__(self, img_path, min_size, max_size, scale_factor_init, num_samples=-1):
    ...
    def load_annotations(self, min_size, max_size, scale_factor_init):
        ...
        self.data_dict["input_sample"] = torch.zeros_like(self.data_dict["real_scale0"])

    def _img2tensor(self, img):
        img = torch.from_numpy(img).to(torch.float32).permute(2, 0, 1).contiguous()
        img = (img / 255 - 0.5) * 2
