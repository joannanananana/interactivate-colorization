...
class BaseModel(nn.Module, metaclass=ABCMeta):
    ...
    ``init_weights``, supporting to initialize models.     ``forward_train``,
    supporting to forward when training.     ``forward_test``, supporting to
    forward when testing.     ``train_step``, supporting to train one step when
    training.
    """
        ...
        for loss_name, loss_value in losses.items():
            if isinstance(loss_value, torch.Tensor):
                ...
                log_vars[loss_name] = sum(
                    _loss.mean() if _loss is not None else 0.0 for _loss in loss_value
                )
            else:
