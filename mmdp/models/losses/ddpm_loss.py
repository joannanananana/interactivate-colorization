...
class DDPMLoss(nn.Module):
        ...
        if not rescale_mode:
            ...
            if rescale_mode == "timestep_weight":
                if sampler is not None and hasattr(sampler, "weight"):
                    ...
                    assert weight is not None and isinstance(weight, torch.Tensor), (
    ...
    @torch.no_grad()
    def quartile_log_collect(
        self, loss, timesteps, total_timesteps, prefix_name, reduction="mean"
    ):
...
class DDPMVLBLoss(DDPMLoss):
    ...
    def __init__(
        ...
        super().__init__(
            rescale_mode, rescale_cfg, log_cfgs, weight, sampler, reduction, loss_name
        )

        self.data_info = self._default_data_info if data_info is None else data_info
        self.data_info_t_0 = (
            self._default_data_info_t_0 if data_info_t_0 is None else data_info_t_0
        )
