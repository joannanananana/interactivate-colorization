...
def gradient_penalty_loss(
    discriminator, real_data, fake_data, mask=None, norm_mode="pixel"
):
    ...
    if norm_mode == "pixel":
        ...
        gradients_penalty = (
            (gradients.reshape(batch_size, -1).norm(2, dim=1) - 1) ** 2
        ).mean()
    else:
        raise NotImplementedError(
...
class GradientPenaltyLoss(nn.Module):
    ...
    **Note for the design of ``data_info``:**
    ...
    def __init__(
        self, loss_weight=1.0, norm_mode="pixel", data_info=None, loss_name="loss_gp"
    ):
        super().__init__()
        self.loss_weight = loss_weight
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.data_info is not None:
            ...
            return r1_gradient_penalty_loss(
                *args, weight=self.loss_weight, norm_mode=self.norm_mode, **kwargs
            )
