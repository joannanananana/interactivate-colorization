...
class PerceptualVGG(nn.Module):
    ...
    def __init__(
        ...
        # blocks = [
        #    int(i) - 1
        #    #for i, o in enumerate(children(_vgg.features))
        #    for i, o in _vgg.features.named_children()
        #    if isinstance(o, nn.MaxPool2d)
        # ]
        # layer_ids = blocks[2:5]
        # print("DEBUG----------------------------------------")
        # print(num_layers, len(_vgg.features))
        # print(layer_ids)
    ...
    def forward(self, x):
        ...
        for name, module in self.vgg_layers.named_children():
            ...
            if name in self.layer_name_list:
                # print('DEBUG------------------------------')
                # print(name, isinstance(module, nn.MaxPool2d))
                output[name] = x.clone()
        return output
...
class PerceptualLoss(nn.Module):
    ...
    def __init__(
        ...
        if (
            self.layer_weights_style is not None
            and self.layer_weights_style != self.layer_weights
        ):
