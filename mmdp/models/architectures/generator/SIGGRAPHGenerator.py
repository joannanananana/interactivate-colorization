...
class SIGGRAPHGenerator(nn.Module):
    ...
    def forward(self, input_A, input_B, mask_B, color_mode=False):
        if color_mode:
            input_A = torch.Tensor(input_A)[None, :, :, :].cuda()
            input_B = torch.Tensor(input_B)[None, :, :, :].cuda()
            mask_B = torch.Tensor(mask_B)[None, :, :, :].cuda()
            mask_B -= 0.5
            conv1_2 = self.model1(
    ...
    def init_weights(
        ...
        if isinstance(pretrained, str):
            ...
            generation_init_weights(self, init_type="xavier", init_gain=0.02)
        else:
            raise TypeError(
