...
from mmdp.models.builder import MODELS
...
class QuanTexSRGANModel(SRGAN):
    """QuanTexSRGANModel model for single image super-resolution.

    Ref:
    QuanTexSRGANModel: Blind Image Super Resolution with Semantic-Aware Quantized Texture Prior,2022
    ...
    def __init__(
        ...
        if self.generator.LQ_stage:
            frozen_module_keywords = self.generator.frozen_module_keywords
    ...
    def train_step(self, data_batch, optimizer):
        ...
        # generator
        fake_g_output, _, l_ae, _, _ = self.generator(lq)
        ...
        if (
            ...
            if self.codebook_loss:
                l_ae *= self.codebook_loss()
                losses["loss_codebook"] = l_ae.mean()
    ...
    def forward_test(
        ...
        output, _, _, _, _ = _model(lq)
        output = output[..., : h_old * 4, : w_old * 4]

        if (
            self.test_cfg is not None
            and self.test_cfg.get("metrics", None)
            and gt is not None
        ):
        ...
        if save_image:
            ...
            if isinstance(iteration, numbers.Number):
                save_path = osp.join(
                    save_path, folder_name, f"{folder_name}-{iteration + 1:06d}.jpg"
                )
