...
def rgb2lab(rgb, l_norm, l_cent, ab_norm):
...
def lab2rgb(lab_rs, opt):
    labx = lab_rs[:, [0], :, :] * opt.get("l_norm") + opt.get("l_cent")
    ab = lab_rs[:, 1:, :, :] * opt.get("ab_norm")
    lab = torch.cat((labx, ab), dim=1)
...
def get_colorization_data(
    ...
    data_lab = rgb2lab(data_raw, l_norm, l_cent, ab_norm)
    data["A"] = data_lab[
        :,
        [
            0,
    ...
    return add_color_patches_rand_gt(data, Batch_size, p=p, num_points=num_points)


def add_color_patches_rand_gt(
    data, bs, p=0.125, num_points=None, use_avg=True, samp="normal"
):
    ...
    for nn in range(N):
        ...
        while cont_cond:
            ...
            P = np.random.choice(bs)  # patch size

            # sample location
            if samp == "normal":  # geometric distribution
                h = int(
                    np.clip(
                        np.random.normal((H - P + 1) / 2.0, (H - P + 1) / 4.0), 0, H - P
                    )
                )
                w = int(
                    np.clip(
                        np.random.normal((W - P + 1) / 2.0, (W - P + 1) / 4.0), 0, W - P
                    )
                )
            ...
            if use_avg:
                ...
                data["hint_B"][nn, :, h : h + P, w : w + P] = torch.mean(
                    torch.mean(
                        data["B"][nn, :, h : h + P, w : w + P], dim=2, keepdim=True
                    ),
                    dim=1,
                    keepdim=True,
                ).view(1, C, 1, 1)
            else:
                data["hint_B"][nn, :, h : h + P, w : w + P] = data["B"][
                    nn, :, h : h + P, w : w + P
                ]
...
def add_color_patch(data, mask, opt, P=1, hw=[128, 128], ab=[0, 0]):
    ...
    data[:, 0, hw[0] : hw[0] + P, hw[1] : hw[1] + P] = 1.0 * ab[0] / opt.ab_norm
    data[:, 1, hw[0] : hw[0] + P, hw[1] : hw[1] + P] = 1.0 * ab[1] / opt.ab_norm
...
def encode_ab_ind(data_ab, opt):
    ...
    data_ab_rs = torch.round(
        (data_ab * opt.get("ab_norm") + opt.get("ab_max")) / opt.get("ab_quant")
    )  # normalized bin number
    data_q = data_ab_rs[:, [0], :, :] * opt.get("A") + data_ab_rs[:, [1], :, :]
    return data_q
...
def decode_ind_ab(data_q, opt):
    ...
    data_a = data_q / opt.get("A")
    data_b = data_q - data_a * opt.get("A")
    ...
    data_ab = (
        (data_ab.type(type_out) * opt.get("ab_quant")) - opt.get("ab_max")
    ) / opt.get("ab_norm")
...
def decode_mean(data_ab_quant, opt):
    ...
    a_range = torch.range(
        -opt.get("ab_max"), opt.get("ab_max"), step=opt.get("ab_quant")
    ).to(data_ab_quant.device)[None, :, None, None]
    a_range = a_range.type(data_ab_quant.type())

    # reshape to AB space
    data_ab_quant = data_ab_quant.view((N, int(opt.get("A")), int(opt.get("A")), H, W))
    ...
    data_ab_inf = torch.cat((data_a_inf, data_b_inf), dim=1) / opt.get("ab_norm")
...
def calculate_psnr_np(img1, img2):
    ...
    SE_map = (1.0 * img1 - img2) ** 2
    cur_MSE = np.mean(SE_map)
    return 20 * np.log10(255.0 / np.sqrt(cur_MSE))


def calculate_psnr_torch(img1, img2):
    SE_map = (1.0 * img1 - img2) ** 2
    cur_MSE = torch.mean(SE_map)
    return 20 * torch.log10(1.0 / torch.sqrt(cur_MSE))
