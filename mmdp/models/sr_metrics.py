...
from mmdp.utils.img_util import img2tensor


def lpips(img1, img2, crop_border=0):
    loss_fn = LPIPS(
        net="alex",
        version="0.1",
        pretrained_model_path="./work_dirs/Quantexsr/lpips/weights/v0.1/alex.pth",
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    loss_fn = loss_fn.to(device)

    img1_tensor = img2tensor(img1).cuda().unsqueeze(0) / 255.0
    img2_tensor = img2tensor(img2).cuda().unsqueeze(0) / 255.0
