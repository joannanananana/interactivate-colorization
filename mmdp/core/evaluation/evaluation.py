...
from mmdp.datasets import build_dataloader, build_dataset
...
def make_vanilla_dataloader(img_path, batch_size, dist=False):
    ...
    dataloader = build_dataloader(
        dataset, samples_per_gpu=batch_size, workers_per_gpu=4, dist=dist, shuffle=True
    )
    return dataloader
...
def offline_evaluation(
    ...
    **kwargs,
    ...
    for metric in metrics:
        assert metric.name not in online_metric_name, (
            "Please eval " f"{metric.name} online"
        )
    ...
    # sample images
    num_exist = len(
        list(mmcv.scandir(samples_path, suffix=(".jpg", ".png", ".jpeg", ".JPEG")))
    )
    ...
    if num_needed > 0 and rank == 0:
        mmcv.print_log(f"Sample {num_needed} fake images for evaluation", "mmdp")
    ...
    fake_dataloader = make_vanilla_dataloader(samples_path, batch_size, dist=ws > 1)
    for metric in metrics:
        mmcv.print_log(f"Evaluate with {metric.name} metric.", "mmdp")
        metric.prepare()
        if rank == 0:
            # prepare for pbar
            total_need = (
                metric.num_real_need
                + metric.num_fake_need
                - metric.num_real_feeded
                - metric.num_fake_feeded
            )
        ...
        if rank == 0:
            ...
            sys.stdout.write("\n")
    if rank == 0:
        table_str = make_metrics_table(
            basic_table_info["train_cfg"],
...
def online_evaluation(
    model, data_loader, metrics, logger, basic_table_info, batch_size, **kwargs
):
    ...
    for metric in vanilla_metrics + recon_metrics:
        metric.prepare()
        max_num_images = max(
            max_num_images, metric.num_real_need - metric.num_real_feeded
        )
    if rank == 0:
        mmcv.print_log(f"Sample {max_num_images} real images for evaluation", "mmdp")
    ...
    max_num_images = (
        0
        if len(vanilla_metrics) == 0
        else max(metric.num_fake_need for metric in vanilla_metrics)
    )
    if rank == 0:
        mmcv.print_log(f"Sample {max_num_images} fake images for evaluation", "mmdp")
        pbar = mmcv.ProgressBar(max_num_images)
    ...
    for metric in special_metrics:
        ...
        fakedata_iterator = iter(
            metric.get_sampler(
                model.module, batch_size, basic_table_info["sample_model"]
            )
        )
        mmcv.print_log(
            f"Sample {metric.num_images} samples for evaluating {metric.name}", "mmdp"
        )
