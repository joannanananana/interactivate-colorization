...
from mmdp.models.architectures import InceptionV3
from mmdp.models.architectures.common import get_module_device
from mmdp.models.losses import PerceptualLoss, gaussian_kld
from mmdp.utils import MMGEN_CACHE_DIR
from mmdp.utils.io_utils import download_from_url
...
def load_inception(inception_args, metric):
    ...
    if not isinstance(inception_args, dict):
        raise TypeError("Receive invalid 'inception_args': " f"'{inception_args}'")
    ...
    if torch.__version__ < "1.6.0":
        mmcv.print_log(
            ...
            f"than '1.6' (now is {torch.__version__})",
            "mmdp",
        )
...
def _load_inception_from_path(inception_path):
    mmcv.print_log(
        "Try to load Tero's Inception Model from " f"'{inception_path}'.", "mmdp"
    )
    try:
        model = torch.jit.load(inception_path)
        mmcv.print_log("Load Tero's Inception Model successfully.", "mmdp")
    except Exception as e:
        model = None
        mmcv.print_log("Load Tero's Inception Model failed. " f"'{e}' occurs.", "mmdp")
    return model


def _load_inception_from_url(inception_url):
    inception_url = inception_url if inception_url else TERO_INCEPTION_URL
    mmcv.print_log(f"Try to download Inception Model from {inception_url}...", "mmdp")
    try:
...
def _load_inception_torch(inception_args, metric):
    ...
    if metric == "FID":
        ...
        mmcv.print_log(
            ...
            "https://github.com/open-mmlab/mmdperation/blob/master/docs/quick_run.md#is.",  # noqa
            "mmdp",
        )
...
def sliced_wasserstein(
    distribution_a, distribution_b, dir_repeats=4, dirs_per_repeat=128
):
...
class Metric(ABC):
    ...
    def feed(self, batch, mode):
        ...
        if mode == "reals":
            ...
            global_end = min(batch_size * ws, self.num_real_need - self.num_real_feeded)
...
class FID(Metric):
    ...
    def __init__(
        ...
        self.inception_net, self.inception_style = load_inception(inception_args, "FID")
        ...
        mmcv.print_log(f"FID: Adopt Inception in {self.inception_style} style", "mmdp")
    ...
    def prepare(self):
        ...
        if self.inception_pkl is not None and mmcv.is_filepath(self.inception_pkl):
            with open(self.inception_pkl, "rb") as f:
                ...
                mmcv.print_log(
                    f"Load reference inception pkl from {self.inception_pkl}", "mmdp"
                )
    ...
    def summary(self):
        ...
        fid, mean, cov = self._calc_fid(
            fake_mean, fake_cov, self.real_mean, self.real_cov
        )
...
class SWD(Metric):
    ...
    def feed_op(self, minibatch, mode):
        ...
        if mode == "reals":
            real_pyramid = laplacian_pyramid(
                minibatch, self.n_pyramids - 1, self.gaussian_k
            )
            ...
            fake_pyramid = laplacian_pyramid(
                minibatch, self.n_pyramids - 1, self.gaussian_k
            )
...
class PR(Metric):
    ...
    def __init__(
        ...
        super().__init__(num_images, image_shape)
        mmcv.print_log("loading vgg16 for improved precision and recall...", "mmdp")
        if os.path.isfile(vgg16_script):
            self.vgg16 = torch.jit.load("work_dirs/cache/vgg16.pt").eval()
            self.use_tero_scirpt = True
        else:
            mmcv.print_log(
                "Cannot load Tero's script module. Use official " "vgg16 instead",
                "mmdp",
            )
    ...
    def check(self):
        if not self.full_dataset:
            ...
            mmcv.print_log(
                f"Test for the full dataset with {self.num_real_feeded}" " real images",
                "mmdp",
            )
    ...
    def summary(self):
        ...
        for name, manifold, probes in [
            ...
            for probes_batch in probes.split(self.row_batch_size):
                ...
                pred.append((distance <= kth).any(dim=1) if rank == 0 else None)
            self._result_dict[name] = float(
                torch.cat(pred).to(torch.float32).mean() if rank == 0 else "nan"
            )
...
class IS(Metric):
    ...
    def __init__(
        ...
        splits=10,
        use_pil_resize=True,
        inception_args=dict(type="StyleGAN", inception_path=TERO_INCEPTION_URL),
    ):
        super().__init__(num_images, image_shape)
        mmcv.print_log("Loading Inception V3 for IS...", "mmdp")
    ...
    def pil_resize(self, x):
        ...
        x_ten = torch.cat([torch.FloatTensor(np.array(x_)[None, ...]) for x_ in x_pil])
    ...
    def summary(self):
        ...
        for k in range(self.splits):
            part = self.preds[
                k
                * (self.num_images // self.splits) : (k + 1)
    ...
    def feed_op(self, minibatch, mode):
        ...
        if factor > 1:
            minibatch = F.interpolate(
                minibatch, size=(256, 256), mode="bilinear", align_corners=False
            )
...
class PPLSampler:
    ...
    def __init__(
        ...
        self.batch_sizes = [batch_size] * n_batch + ([resid] if resid > 0 else [])
    ...
    def __next__(self):
        ...
        if self.space == "W":
            ...
            latent_e1 = torch.lerp(latent_t0, latent_t1, lerp_t[:, None] + self.epsilon)
            latent_e = torch.stack([latent_e0, latent_e1], 1).view(*latent.shape)
            image = self.generator(
                [latent_e], input_is_latent=True, injected_noise=injected_noise
            )
        else:
            latent_t0, latent_t1 = inputs[::2], inputs[1::2]
            latent_e0 = slerp(latent_t0, latent_t1, lerp_t[:, None])
            latent_e1 = slerp(latent_t0, latent_t1, lerp_t[:, None] + self.epsilon)
            latent_e = torch.stack([latent_e0, latent_e1], 1).view(*inputs.shape)
            image = self.generator(
                [latent_e], input_is_latent=False, injected_noise=injected_noise
            )
...
class GaussianKLD(Metric):
    ...
    def __init__(self, num_images, base="e", reduction="batchmean"):
        ...
        assert reduction in ["sum", "batchmean", "mean"], (
            "We only support reduction for 'batchmean', 'sum' " "and 'mean'"
        )
        assert base in ["e", "2"], "We only support log_base for 'e' and '2'"
        self.reduction = reduction
        self.num_fake_feeded = self.num_images
        self.cal_kld = partial(gaussian_kld, weight=1, reduction="none", base=base)
    ...
    def feed_op(self, batch, mode):
        ...
        # check required keys
        require_keys = ["mean_pred", "mean_target", "logvar_pred", "logvar_target"]
        if any([k not in batch for k in require_keys]):
            raise KeyError(
                f"The input dict must require {require_keys} at "
