...
def extract_inception_features(
    dataloader, inception, num_samples, inception_style="pytorch"
):
...
def get_pyramid_layer(image, gaussian_k, direction="down"):
    ...
    multiband = [
        F.conv2d(
            ...
            gaussian_k,
            padding=2,
            stride=1 if direction == "up" else 2,
        )
        for i in range(3)
    ]
...
def laplacian_pyramid(original, n_pyramids, gaussian_k):
    ...
    for i in range(len(pyramids) - 1):
        diff = pyramids[i] - get_pyramid_layer(pyramids[i + 1], gaussian_k, "up")
...
def compute_pr_distances(
    row_features, col_features, num_gpus, rank, col_batch_size=10000
):
