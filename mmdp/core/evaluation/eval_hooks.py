...
class GenerativeEvalHook(Hook):
    ...
    def __init__(
        ...
        if isinstance(interval, int):
            ...
            raise TypeError(
                ...
                f"receive {type(self.interval)} instead."
            )
        ...
        if self.save_best_ckpt:
            not_supported = set(self.best_metric) - set(self._supported_best_metrics)
            assert (
                len(not_supported) == 0
        ...
        if self.save_best_ckpt:
            ...
            for name in self.best_metric:
                ...
                self._curr_best_score[name] = self.init_value_map[self.rule[name]]
    ...
    def after_train_iter(self, runner):
        ...
        max_real_num_images = max(
            metric.num_images - metric.num_real_feeded for metric in self.metrics
        )
        # define mmcv progress bar
        if rank == 0 and max_real_num_images > 0:
            mmcv.print_log(
                f"Sample {max_real_num_images} real images for evaluation", "mmdp"
            )
        ...
        if rank == 0:
            mmcv.print_log(
                f"Sample {max_num_images} fake images for evaluation", "mmdp"
            )
    ...
    def _save_best_ckpt(self, runner, new_score, metric_name):
        ...
        if self.compare_func[metric_name](
            new_score, self._curr_best_score[metric_name]
        ):
    ...
    def after_train_iter(self, runner):
        ...
        for metric in self.metrics:
            if metric.num_real_feeded >= metric.num_real_need:
                continue
            mmcv.print_log(f"Feed reals to {metric.name} metric.", "mmdp")
            # feed in real images
            for data in self.dataloader:
...
class EvalIterHook(Hook):
    ...
    def after_train_iter(self, runner):
        ...
        if not self.every_n_iters(runner, self.interval):
            return
        runner.log_buffer.clear()
        from mmdp.apis import single_gpu_test
    ...
    def evaluate(self, runner, results):
        ...
        """
        eval_res = self.dataloader.dataset.evaluate(results, logger=runner.logger)
...
class DistEvalIterHook(EvalIterHook):
    ...
    def after_train_iter(self, runner):
        ...
        if not self.every_n_iters(runner, self.interval):
            return
        runner.log_buffer.clear()
        from mmdp.apis import multi_gpu_test
