...
from mmdp.apis import set_random_seed
from mmdp.core import build_metric
from mmdp.core.evaluation import make_metrics_table, make_vanilla_dataloader
from mmdp.datasets import build_dataloader, build_dataset
from mmdp.models import build_model
from mmdp.models.translation_models import BaseTranslationModel
...
def single_gpu_online_evaluation(
    ...
    # sample images
    max_num_images = (
        0 if len(metrics) == 0 else max(metric.num_fake_need for metric in metrics)
    )
...
def main():
    ...
    mmcv.print_log(f"Sampling model: {args.sample_model}", "mmdp")
    ...
    if args.eval:
        if args.eval[0] == "none":
            ...
            metrics = [build_metric(cfg.metrics[metric]) for metric in args.eval]
    else:
    ...
    if args.online:
        single_gpu_online_evaluation(
            model, data_loader, metrics, logger, basic_table_info, args.batch_size
        )
    else:
