...
from mmdp.apis import set_random_seed  # isort:skip  # noqa
from mmdp.models import build_model  # isort:skip  # noqa
...
def parse_args():
    ...
    parser.add_argument(
        "--num-samples", type=int, default=10, help="the number of synthesized samples"
    )
...
def main():
    ...
    # load ckpt
    mmcv.print_log(f"Loading ckpt from {args.checkpoint}", "mmdp")
    ...
    for sample_iter in range(args.num_samples):
        ...
        if args.save_prev_res:
            ...
            for i, img in enumerate(prev_res_list):
                ...
                mmcv.imwrite(
                    img,
                    os.path.join(
                        args.samples_path, f"stage{i}", f"rand_sample_{sample_iter}.png"
                    ),
                )
        # just store the final result
            ...
            mmcv.imwrite(
                img, os.path.join(args.samples_path, f"rand_sample_{sample_iter}.png")
            )
