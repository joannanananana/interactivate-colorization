...
from mmdp.core.evaluation.metric_utils import extract_inception_features  # isort:skip  # noqa
from mmdp.datasets import (UnconditionalImageDataset, build_dataloader,  # isort:skip  # noqa
                            build_dataset)  # isort:skip  # noqa
from mmdp.models.architectures import InceptionV3  # isort:skip  # noqa
# yapf: enable
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "automatically use all samples in the subset"
        ),
    )
    parser.add_argument(
        "--no-shuffle", action="store_true", help="not use shuffle in data loader"
    )
    parser.add_argument(
        "--subset",
        default="test",
    ...
    parser.add_argument(
        "--inception-pth", type=str, default="work_dirs/cache/inception-2015-12-05.pt"
    )
    ...
    if args.pipeline_cfg is not None:
        ...
        if isinstance(args.size, list) and len(args.size) == 2:
            ...
            raise TypeError(f"args.size mush be int or tuple but got {args.size}")

        pipeline = [
            dict(type="LoadImageFromFile", key="real_img"),
            dict(type="Resize", keys=["real_img"], scale=size, keep_ratio=False),
            dict(
                type="Normalize",
                keys=["real_img"],
                mean=[127.5] * 3,
        ...
        if args.flip:
            pipeline.insert(
                1, dict(type="Flip", keys=["real_img"], direction="horizontal")
            )
    ...
    if args.inception_style == "stylegan":
        ...
        print_log("Adopt Inception network in StyleGAN", "mmdp")
    else:
    ...
    if args.num_samples == -1:
        print_log("Use all samples in subset", "mmdp")
        num_samples = len(dataset)
    else:
        num_samples = args.num_samples

    features = extract_inception_features(
        data_loader, inception, num_samples, args.inception_style
    ).numpy()

    # sanity check for the number of features
    assert features.shape[0] == num_samples, "the number of features != num_samples"
    print_log(f"Extract {num_samples} features", "mmdp")
    ...
    with open(osp.join(args.pkl_dir, args.pklname), "wb") as f:
        pickle.dump(
            {"mean": mean, "cov": cov, "size": num_samples, "name": args.pklname}, f
        )
