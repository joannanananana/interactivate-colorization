...
train_pipeline = [
    dict(
        ...
        domain_a=domain_a,
        domain_b=domain_b,
        paired=False,  # True for paired, False for single
        flag="color",
    ),
    dict(
        type="Resize",
...
data = dict(
    ...
    workers_per_gpu=4,
    shuffle=True,
    train=dict(dataroot=dataroot, pipeline=train_pipeline, phase="rjzdscs1"),
    val=dict(
        dataroot=dataroot, pipeline=test_pipeline, phase="rjzdscs2"
    ),  # testdir='val'),
    test=dict(dataroot=dataroot, pipeline=test_pipeline, phase="rjzdscs2"),
)  # , testdir='val'))

optimizer = dict(
    # generator=dict(type='Adam', lr=1e-4, betas=(0.5, 0.999)),
    # discriminator=dict(type='Adam', lr=1e-4, betas=(0.5, 0.999)))
    generator=dict(type="Adam", lr=1e-6, betas=(0.0, 0.999), weight_decay=1e-3),
    discriminator=dict(type="Adam", lr=2.5e-6, betas=(0.0, 0.999), weight_decay=1e-3),
)
